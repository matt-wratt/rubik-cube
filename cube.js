...
export function createCube(camera) {
  ...
  nodes.forEach((node) => {
    ...
    node.positionX = () => node.three.localToWorld(new THREE.Vector3(0, 0, 0)).x
    node.positionY = () => node.three.localToWorld(new THREE.Vector3(0, 0, 0)).y
    node.positionZ = () => node.three.localToWorld(new THREE.Vector3(0, 0, 0)).z
  ...
  loader.load(
    'three.js/examples/fonts/helvetiker_regular.typeface.json',
    (font) => {
      const size = 16
      const height = 1
